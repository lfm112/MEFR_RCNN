...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def normalize(self, image):
        ...
        # RGB 通道的均值和标准差
        rgb_mean = torch.as_tensor(self.image_mean, dtype=dtype, device=device)  # 形状 [3]
        rgb_std = torch.as_tensor(self.image_std, dtype=dtype, device=device)  # 形状 [3]

        # 红外通道的均值和标准差
        ir_mean = torch.as_tensor(self.image_mean, dtype=dtype, device=device).view(-1, 1, 1)  # 形状 [1, 1, 1]

        ir_std = torch.as_tensor(self.image_std, dtype=dtype, device=device).view(-1, 1, 1)  # 形状 [1, 1, 1]

        # 分别标准化 RGB 和红外通道
        rgb_normalized = (image[:3, :, :] - rgb_mean[:, None, None]) / rgb_std[:, None, None]

        ir_normalized = (image[3, :, :] - ir_mean[0]) / ir_std[0]



        # 合并标准化后的结果
        normalized_image = torch.cat([rgb_normalized, ir_normalized.unsqueeze(0)], dim=0)
